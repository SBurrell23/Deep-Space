...
export const SHIP_ACHIEVEMENTS = {
  kestrel: [
    { id: 'kestrel_1', icon: 'icon_star', name: 'Federation Standard', desc: 'Reach sector 5 with the Kestrel.', check: c => c.run.sectorIndex >= 4 },
    { id: 'kestrel_2', icon: 'icon_sys_weapons', name: 'Improvise', desc: 'Win a fight with the Kestrel using only Basic Lasers.', check: c => c.event === 'combatVictory' && c.ship.weapons.every(w => w.weaponId === 'laser_basic') },
    { id: 'kestrel_3', icon: 'icon_trophy', name: 'Home Again', desc: 'Beat the flagship with the Kestrel.', check: c => c.event === 'bossKilled' },
  ],
  torus: [
    { id: 'torus_1', icon: 'icon_repair', name: 'Tank', desc: 'Survive a run with the Torus down to 3 hull and recover to full.', check: c => c.run.stats.recoveredFromCritical },
    { id: 'torus_2', icon: 'icon_exit', name: 'Slow And Steady', desc: 'Reach sector 6 with the Torus.', check: c => c.run.sectorIndex >= 5 },
    { id: 'torus_3', icon: 'icon_trophy', name: 'Hauler', desc: 'Beat the flagship with the Torus.', check: c => c.event === 'bossKilled' },
  ],
  mantis: [
    { id: 'mantis_1', icon: 'icon_skull', name: 'Butcher', desc: 'Kill 15 enemy crew in boarding actions.', check: c => (c.run.stats.boardingKills || 0) >= 15 },
    { id: 'mantis_2', icon: 'icon_crew', name: 'Prize Crew', desc: 'Capture three ships by wiping out their crews.', check: c => (c.run.stats.captures || 0) >= 3 },
    { id: 'mantis_3', icon: 'icon_trophy', name: 'Apex', desc: 'Beat the flagship with the Mantis Raider.', check: c => c.event === 'bossKilled' },
  ],
  engi: [
    { id: 'engi_1', icon: 'icon_sys_drones', name: 'Hands Off', desc: 'Destroy a ship using only drones.', check: c => c.event === 'combatVictory' && c.playerWeaponDamage === 0 },
    { id: 'engi_2', icon: 'icon_check', name: 'Systems Nominal', desc: 'Reach sector 5 with the Engi Cruiser undamaged.', check: c => c.run.sectorIndex >= 4 && c.ship.hull === c.ship.maxHull },
    { id: 'engi_3', icon: 'icon_trophy', name: 'Optimised', desc: 'Beat the flagship with the Engi Cruiser.', check: c => c.event === 'bossKilled' },
  ],
  zoltan: [
    { id: 'zoltan_1', icon: 'icon_power', name: 'Living Reactor', desc: 'Have four Zoltan aboard at once.', check: c => c.ship.crew.filter(x => !x.dead && x.race === 'zoltan').length >= 4 },
    { id: 'zoltan_2', icon: 'icon_sys_battery', name: 'Diplomatic Immunity', desc: 'Reach sector 6 with the Zoltan ship.', check: c => c.run.sectorIndex >= 5 },
    { id: 'zoltan_3', icon: 'icon_trophy', name: 'Enlightened', desc: 'Beat the flagship with the Zoltan Ambassador.', check: c => c.event === 'bossKilled' },
  ],
  stealth: [
    { id: 'stealth_1', icon: 'icon_evade', name: 'Ghost', desc: 'Win a fight without ever being hit, in a Stealth ship.', check: c => c.event === 'combatVictory' && c.hullLost === 0 },
    { id: 'stealth_2', icon: 'icon_sys_cloaking', name: 'Now You See Me', desc: 'Reach sector 5 with no shield system installed.', check: c => c.run.sectorIndex >= 4 && !c.ship.systems.shields },
    { id: 'stealth_3', icon: 'icon_trophy', name: 'Unseen', desc: 'Beat the flagship with a Stealth ship.', check: c => c.event === 'bossKilled' },
  ],
  rock: [
    { id: 'rock_1', icon: 'icon_missile', name: 'Bombardment', desc: 'Fire 60 missiles in one run.', check: c => (c.run.stats.missilesFired || 0) >= 60 },
    { id: 'rock_2', icon: 'icon_sys_engines', name: 'Unmoved', desc: 'Reach sector 5 with engines never upgraded past level 1.', check: c => c.run.sectorIndex >= 4 && (c.ship.systems.engines?.level ?? 1) <= 1 },
    { id: 'rock_3', icon: 'icon_trophy', name: 'Immovable', desc: 'Beat the flagship with a Rock ship.', check: c => c.event === 'bossKilled' },
  ],
  slug: [
    { id: 'slug_1', icon: 'icon_sys_mindcontrol', name: 'Mind Games', desc: 'Mind control 8 enemy crew in one run.', check: c => (c.run.stats.mindControls || 0) >= 8 },
    { id: 'slug_2', icon: 'icon_sys_sensors', name: 'Blind Navigation', desc: 'Reach sector 5 with no sensors installed.', check: c => c.run.sectorIndex >= 4 && !c.ship.systems.sensors },
    { id: 'slug_3', icon: 'icon_trophy', name: 'Whisper', desc: 'Beat the flagship with a Slug ship.', check: c => c.event === 'bossKilled' },
  ],
  crystal: [
    { id: 'crystal_1', icon: 'icon_lock', name: 'Lockdown', desc: 'Seal 6 rooms with Crystal lockdown in one run.', check: c => (c.run.stats.lockdowns || 0) >= 6 },
    { id: 'crystal_2', icon: 'icon_hull', name: 'Ancient', desc: 'Reach sector 6 with a Crystal ship.', check: c => c.run.sectorIndex >= 5 },
    { id: 'crystal_3', icon: 'icon_trophy', name: 'Enduring', desc: 'Beat the flagship with a Crystal ship.', check: c => c.event === 'bossKilled' },
  ],
  nomad: [
    { id: 'nomad_1', icon: 'icon_scrap', name: 'Magpie', desc: 'Earn 600 scrap in a single run.', check: c => (c.run.stats.scrapEarned || 0) >= 600 },
    { id: 'nomad_2', icon: 'icon_sys_nanoforge', name: 'Self-Repairing', desc: 'Repair 25 hull with the Nanoforge in one run.', check: c => (c.run.stats.nanoforgeRepairs || 0) >= 25 },
    { id: 'nomad_3', icon: 'icon_trophy', name: 'Scavenger King', desc: 'Beat the flagship with the Nomad.', check: c => c.event === 'bossKilled' },
  ],
};
